...
class EdgeCenterReduceNorm(nnx.Module):
    ...
    def __init__(
        ...
        self.mean = nnx.Variable(jnp.zeros(n_features))  # TODO : regarder s'il ne vaut pas mieux utiliser nnx.BatchStat
    ...
    def __call__(self, x: jax.Array):
        ...
        if (self.updates < self.update_limit) or (not self.use_running_average):
            ...
            if is_batched:
                current_mean, current_var = x.mean(axis=(0, 1)), x.var(axis=(0, 1))
            else:
                current_mean, current_var = x.mean(axis=0), x.var(axis=0)

            self.mean.value = self.beta_1 * self.mean.value + (1 - self.beta_1) * current_mean
            self.var.value = self.beta_2 * self.var.value + (1 - self.beta_2) * current_var

        # Correct bias
        mean_hat = self.mean.value / (1 - self.beta_1**self.updates)
        var_hat = self.var.value / (1 - self.beta_2**self.updates)

        return (x - mean_hat) / (jnp.sqrt(var_hat) + self.epsilon)
