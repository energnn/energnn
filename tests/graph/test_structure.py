...
def test_edge_structure_init():
    ...
    es = HyperEdgeSetStructure(address_list=address_list, feature_list=feature_list)
    assert es.address_list == address_list
    assert es.feature_list == feature_list
    assert es["address_list"] == address_list
    assert es["feature_list"] == feature_list

    es_none = HyperEdgeSetStructure(address_list=None, feature_list=None)
...
def test_edge_structure_from_list():
    address_list = ["id"]
    feature_list = ["val"]
    es = HyperEdgeSetStructure.from_list(address_list=address_list, feature_list=feature_list)
...
def test_graph_structure_init():
    es1 = HyperEdgeSetStructure(address_list=["from", "to"], feature_list=["val"])
    es2 = HyperEdgeSetStructure(address_list=["id"], feature_list=["state"])
...
def test_graph_structure_from_dict():
    es = HyperEdgeSetStructure(address_list=["id"], feature_list=None)
    edge_dict = {"source": es}
...
def test_structure_inheritance_dict():
    es = HyperEdgeSetStructure(address_list=[], feature_list=[])
