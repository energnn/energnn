...
def test_feature_flat_array_concatenation_order_and_shape():
    ...
    e1 = HyperEdgeSet.from_dict(address_dict={"a": np.array([0, 1])}, feature_dict={"f1": np.array([1.0, 2.0])})
    e2 = HyperEdgeSet.from_dict(address_dict={"b": np.array([0, 1])}, feature_dict={"f2": np.array([3.0, 4.0])})
    g = Graph.from_dict(hyper_edge_set_dict={"A": e1, "B": e2}, registry=np.arange(5, dtype=np.float32))
...
def test_quantiles_match_numpy_graph_quantiles():
    ...
    edge = HyperEdgeSet.from_dict(address_dict={"a": np.arange(arr.size)}, feature_dict={"f0": arr})
    g = Graph.from_dict(hyper_edge_set_dict={"etype": edge}, registry=np.arange(10, dtype=np.float32))
