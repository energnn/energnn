...
class StubProblem(SelfSupervisedProblem):
    ...
    def get_context(self, step_with_metrics=False):
        return make_dummy_graph_mock(edges={}), {}

    def get_zero_decision(self, step_with_metrics=False):
        return make_dummy_graph_mock(edges={}), {}

    def get_gradient(self, *, decision, step_with_metrics=False, step: int | None = None):
        return decision, {}

    def get_score(self, *, decision, step_with_metrics=False, step: int | None = None):
        return 0.0, {}
...
def test_get_methods_return_tuple_and_info():
    """Check each abstract method returns (Graph, dict) or (float, dict) and handles step_with_metrics flag."""

    class P(StubProblem):
        def get_context(self, step_with_metrics=False, step=None):
        ...
        def get_score(self, *, decision, step_with_metrics=False, step=None):
            metric = 3.14
            info = {"minfo": "m"} if step_with_metrics else {}
            return metric, info

    p = P()
    ctx, info0 = p.get_context(step_with_metrics=False)
    assert isinstance(ctx, Graph)
    assert info0 == {}

    _, info1 = p.get_context(step_with_metrics=True)
    assert info1 == {"cinfo": True}

    grad, g_info = p.get_gradient(decision=ctx, step_with_metrics=True)

    assert isinstance(grad, Graph)
    assert g_info == {"ginfo": "ok"}

    metric, m_info = p.get_score(decision=ctx, step_with_metrics=True)
...
def test_supervised_problem_interface():
    ...
    class P(SupervisedProblem):
        ...
        def get_loss(self, *, decision, step_with_metrics=False, step=None):
    ...
    p = P(val=42.0)
    loss, info = p.get_loss(decision=make_dummy_graph_mock(edges={}), step_with_metrics=True)
    assert loss == 42.0
